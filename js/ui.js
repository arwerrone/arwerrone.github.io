...
function toggleLoading(isLoading) {
  ...
  if (isLoading) {
    let tochange = document.querySelector("#botao");
    let ico = document.createElement("i");
    ico.className = "fas fa-hourglass-half";
    let tex = document.createTextNode(" Loading...");
    while (tochange.hasChildNodes()) {
      tochange.removeChild(tochange.firstChild);
    }
    tochange.appendChild(ico);
    tochange.appendChild(tex);
    tochange.setAttribute("disabled", "disabled");
  }
  if (!isLoading) {
    let tochange = document.querySelector("#botao");
    let ico = document.createElement("i");
    ico.className = "fas fa-search";
    let tex = document.createTextNode(" Search");
    while (tochange.hasChildNodes()) {
      tochange.removeChild(tochange.firstChild);
    }
    tochange.appendChild(ico);
